...
#Fivebit version 0.1 [21/6/2013] by Dankoozie
...
class Substitute:
    ...
    def sub(self,str_in,usedict = True):
        ...
        for a in str_in:
            ...
            elif(ord(a) in range(65,97)): #Upper case and [\]^_`
                cl.extend([27,ord(a) - 65])

            elif(ord(a) == 32): #Space
                cl.append(26)

                if(usedict): # Check if word exists in dictionary
                    wordcode = search_dict("".join(word)[:-1])             
                    if(wordcode != -1):
                         cl = cl[:len(cl) - (len(word) -1)]
                         cl.extend([30,wordcode >> 5,wordcode & 31])
                    word = []

            elif(ord(a) in range(33,65)): cl.extend([28,ord(a) - 33])
            else:
                #UTF-8
