...
#Fivebit [15/02/2016] by Dankoozie
...
class threadencode(threading.Thread):
    def __init__(self):
...
def all_instance(lst,size):
    od = []
    c=0
    while c < len(lst) - (size-1):
        od.append(tuple(lst[c:c+size]))
        c += 1
    return(od)
         

def findrepeat(lst,n = 3,bso = False):
    #bso = Return bytes saved by compressing instead of number of repeats
    rl = {}
    od = []
    c = 0
    while c < len(lst) - (n-1):
    ...
    for k in rl:
        if(not bso):
            if(rl[k] > 1): od.append((k,rl[k]))
        else:
            if(rl[k] > 1): od.append((k,(len(rl)*(rl[k]-1)) - 2))
    return od

def findbs(lst):
    i = 3
    while True:
        rs = findrepeat(lst,i,True)
        i += 1
        if(rs == {}): break
