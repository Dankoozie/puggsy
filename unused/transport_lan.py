...
class lancontact():
    def __init__(self,nick,addr,port,uid,status,maincontact = -1):
        self.nfid = None
        self.b_nick = nick
        self.b_addr = addr
        self.b_port = port
    ...
    def update_maincontact(self):
        ...
        if(self.maincontact == -1):
            mc = contacts.Contact(str(self.b_nick,'UTF-8'),self.b_status)
            self.maincontact = mc.nfid

        cm = contacts.Contactlist[self.maincontact]
        #cm.nick = self.b_nick
        cm.presence = self.b_status
        cm.ui_update()
    ...
    def addself(self):
        ...
        contacts.Contactlist[self.maincontact].li = self.b_uid
